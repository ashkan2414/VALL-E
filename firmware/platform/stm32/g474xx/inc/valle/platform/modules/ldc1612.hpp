...
namespace valle
{
    ...
    public:
        LDC161XSensorDriver() = delete;

        explicit LDC161XSensorDriver(DeviceRef<I2CSlaveDeviceT>&& slave_device) : m_i2c(std::move(slave_device))
        {
        }

        [[nodiscard]] bool init(const LDC161XSensorConfig<tkNumChannels>& config)
        {
            m_config             = get_raw_config(config);
        ...
        void reset()
        {
            ...
            register_write_blocking(LDC161XReg::kDeviceReset, reset_reg.serialize(), skDefaultTimeout);
        ...
        [[nodiscard]] uint16_t read_manufacturer_id(const std::chrono::milliseconds timeout_ms = skDefaultTimeout)
        {
            ...
            register_read_blocking(LDC161XReg::kReadManufacturerID, manufacturer_id, timeout_ms);
        ...
        {
            uint16_t device_id;
            register_read_blocking(LDC161XReg::kReadDeviceID, device_id, timeout_ms);
        ...
        void configure()
        {
            ...
            // MUX CONFIG
            register_write_blocking(LDC161XReg::kMuxConfig, m_config.mux_config.serialize(), skDefaultTimeout);

            // ERROR CONFIG
            register_write_blocking(LDC161XReg::kErrorConfig, m_config.error_config.serialize(), skDefaultTimeout);

            // DEVICE CONFIG (exit sleep mode last)
            register_write_blocking(LDC161XReg::kConfig, m_config.device_config.serialize(), skDefaultTimeout);
        }
        ...
        void configure_channel()
        {
            ...
            // Write CLOCK DIVIDER
            register_write_blocking(LDC161XTraits::skChannelClockDividerReg<tkChannel>,
                                    chan_config.clock_divider.serialize(),
                                    skDefaultTimeout);

            // Write RCOUNT
            register_write_blocking(
            ...
            // Write SETTLE COUNT
            register_write_blocking(LDC161XTraits::skChannelSettleCountReg<tkChannel>,
        ...
        {
            uint16_t status_raw;
            register_read_blocking(LDC161XReg::kStatus, status_raw, timeout_ms);
        ...
        [[nodiscard]] uint32_t read_data_raw(const std::chrono::milliseconds timeout_ms = skDefaultTimeout)
        {
            ...
            register_read_blocking(LDC161XTraits::skChannelDataMSBReg<tkChannel>, data_msb, timeout_ms);
            register_read_blocking(LDC161XTraits::skChannelDataLSBReg<tkChannel>, data_lsb, timeout_ms);
        ...
        [[nodiscard]] I2CBlockingTransactionError register_write_blocking(LDC161XReg                reg,
                                                                          const TValue&             value,
        ...
        [[nodiscard]] I2CBlockingTransactionError register_write_blocking(
            LDC161XReg reg, const std::span<const std::byte, tkWriteBytes>& data, std::chrono::milliseconds timeout_ms)
        {
        ...
        template <uint8_t tkWriteBytes>
        [[nodiscard]] I2CBlockingTransactionError register_write_blocking(
            const std::span<const std::byte, tkWriteBytes>& write_data, std::chrono::milliseconds timeout_ms)
        {
            // Build the command sequence (Single Write)
            auto write_commands = I2CTransactionGenerator<>::begin().write(std::span(write_data)).build();

            // Submit
            return m_i2c->submit_transaction_blocking(write_commands, timeout_ms);
        }

        template <typename TValue>
        [[nodiscard]] I2CBlockingTransactionError register_read_blocking(LDC161XReg                reg,
                                                                         TValue&                   value_out,
                                                                         std::chrono::milliseconds timeout_ms)
        {
        ...
        [[nodiscard]] I2CBlockingTransactionError register_read_blocking(
            LDC161XReg reg, const std::span<std::byte, tkReadBytes>& data_out, std::chrono::milliseconds timeout_ms)
        {
            ...
            // Submit
            return m_i2c->submit_transaction_blocking(read_commands, timeout_ms);
        }
    ...
    private:
        ...
        {
            return static_cast<uint16_t>((coil_q_factor * fref_mhz) / (16.0f * fsensor_mhz)) + 1;
        }
        ...
        [[nodiscard]] static constexpr std::array<LDC161XChannelConfigRaw, tkNumChannels> get_channel_config(
            const LDC161XSensorConfig<tkNumChannels>& config)
        {
