...
def process_enum(match):
    ...
    for k, v in items:
        if not re.match(r"^-?\d+$", v):
            all_numbers = False
        if (
            not v.startswith("LL_")
            and not v.startswith("UART_")
            and not v.startswith("USART_")
            and not v.startswith("HAL_")
        ):
            all_ll = False
...
files = glob.glob(
    "C:/Users/ebrah/Documents/Documents/Source/Capstone/VALL-E/firmware/src/platform/stm32/g474xx/inc/valle/platform/hardware/**/interface.hpp",
    recursive=True,
)
for f in files:
    modify_file(f)
