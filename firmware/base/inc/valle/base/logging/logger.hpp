...
namespace valle
{
    ...
    public:
        ...
        {
            if constexpr (kLoggingConfig.kEnabled && kShouldLog<tkLevel>)
            {
                // Fixed-size buffer to avoid dynamic allocation
                std::array<char, kLoggingConfig.kMaxMessageLength> buffer{};
                auto                                               current_buffer =
                    std::span<char>(buffer.data(), buffer.size() - 1);  // Reserve space for null terminator

                // Format location
                if constexpr (kLoggingConfig.kPrintSourceLocation)
                {
