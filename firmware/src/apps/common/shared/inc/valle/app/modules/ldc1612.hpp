...
namespace valle::app
{
    ...
    public:
        ...
        template <typename TValue>
        [[nodiscard]] bool register_read_blocking(LDC161XReg reg, TValue& value_out, system::TimeoutMillis timeout_ms)
        {
    ...
    public:
        ...
        [[nodiscard]] uint16_t read_manufacturer_id(const system::TimeoutMillis timeout_ms = skDefaultTimeout)
        {
        ...
        [[nodiscard]] uint16_t read_device_id(const system::TimeoutMillis timeout_ms = skDefaultTimeout)
        {
            uint16_t device_id;
        ...
        [[nodiscard]] LDC161XStatusReg read_status(const system::TimeoutMillis timeout_ms = skDefaultTimeout)
        {
            uint16_t status_raw;
        ...
        template <uint8_t tkChannel>
            requires(tkChannel >= 0 && tkChannel < tkNumChannels)
        [[nodiscard]] uint32_t read_data_raw(const system::TimeoutMillis timeout_ms = skDefaultTimeout)
        {
        ...
        template <uint8_t tkChannel>
            requires(tkChannel >= 0 && tkChannel < tkNumChannels)
        [[nodiscard]] double read_frequency(const system::TimeoutMillis timeout_ms = skDefaultTimeout)
        {
