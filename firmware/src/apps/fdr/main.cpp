...
namespace valle::app
{
    ...
    float compute_intake_valve_current(const EngineStroke stroke, float pos_norm)
    {
        if (stroke == EngineStroke::kIntake)
        {
            ...
            {
                g_drivers.test_gpio.write(true);
                return kOpenCurrent;
            }
        }

        g_drivers.test_gpio.write(false);
        return kCloseCurrent;
    }
    ...
    void main()
    {
        ...
        while (true)
        {
            TimingContext::run_within_period_ms([]() { run_control_loop(); }, 3);  // ~300Hz control loop
        }
    }
}  // namespace valle::app