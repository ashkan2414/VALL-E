...
namespace valle::app
{
    ...
    void on_current_sensor_adc_reading()
    {
        ...
        g_drivers.test_gpio.toggle();

        // First read the sampled current sensor and convert
        g_drivers.current_sensor.get_adc().on_data_available();

        const float measured_current = g_drivers.current_sensor.read_amps();

        g_current_response_collector.push_data(CurrentResponseData{
            .timestamp        = system::MicrosClock::now(),
            .target_current   = g_drivers.vca_controller.get_target_current(),
            .measured_current = measured_current,
        });

        // Then run VCA control loop
        g_last_duty_cycle = g_drivers.vca_controller.run_ctrl_loop(measured_current);
    }
