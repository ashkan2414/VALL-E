...
public:
    ...
    static inline void init_output_channel(const uint32_t output, const HRTIMOutputChannelConfig& config)
    {
        ...
        LL_HRTIM_OUT_SetFaultState(ControllerTraitsT::skInstance, output, static_cast<uint32_t>(config.fault_state));
    }
};
...
public:
    ...
    [[nodiscard]] static inline uint64_t get_hrck_freq_hz()
    {
        ...
        const uint32_t prescaler  = LL_HRTIM_TIM_GetPrescaler(ControllerTraitsT::skInstance, TimerTraitsT::skTimerIdx);
