...
struct BiQuadCoeffs
{
    ...
    inline BiQuadCoeffs(TBRange&& b_src, TARange&& a_src)
    {
    ...
    inline BiQuadCoeffs(Eigen::Vector<T, 3>&& b_coeffs, Eigen::Vector<T, 3>&& a_coeffs)
        : b(std::move(b_coeffs)), a(std::move(a_coeffs))
    {
    }

    inline BiQuadCoeffs(const Eigen::Vector<T, 3>& b_coeffs, const Eigen::Vector<T, 3>& a_coeffs)
        : b(b_coeffs), a(a_coeffs)
    {
    }

    inline BiQuadCoeffs operator*(T gain) const
    {
        BiQuadCoeffs result;
        result.b = b * gain;
...
struct BiQuadCoeffsNorm
{
    ...
    template <std::ranges::range TBRange, std::ranges::range TARange>
    inline BiQuadCoeffsNorm(TBRange&& b_src, TARange&& a_src)
    {
        // Compile-Time Checks (Static Assertions)
        // -----------------------------------------------------------
    ...
    inline BiQuadCoeffsNorm(Eigen::Vector<T, 3>&& b_coeffs, Eigen::Vector<T, 2>&& a_coeffs)
        : b(std::move(b_coeffs)), a(std::move(a_coeffs))
    {
    }

    inline BiQuadCoeffsNorm(const Eigen::Vector<T, 3>& b_coeffs, const Eigen::Vector<T, 2>& a_coeffs)
        : b(b_coeffs), a(a_coeffs)
    {
    }

    BiQuadCoeffsNorm(const BiQuadCoeffs<T>& coeffs)
    {
...
template <typename T>
T calculate_biquad_df1_norm(const BiQuadCoeffsNorm<T>& coeffs, Eigen::Vector<T, 2>& x, Eigen::Vector<T, 2>& y, T input)
{
    ...
    const T dot_hist = precise_dot<T>(coeffs.b.template tail<2>(), x);
    T       ff       = 0;
    if constexpr (std::is_floating_point_v<T>)
    {
        ff = std::fma(input, coeffs.b[0], dot_hist);
    }
    else
    {
        ff = (input * coeffs.b[0]) + dot_hist;
    }

    // 2. Feed-Back (Poles): Dot product of 'a' coefficients and output history
    const T fb = precise_dot(coeffs.a, y);
...
private:
    ...
    Eigen::Vector<T, 2> m_x;
    Eigen::Vector<T, 2> m_y;
...
public:
    ...
    BiQuadDF1Norm(TCoeffs&& coeffs, T gain = static_cast<T>(1.0))
...
public:
    BiQuadDF1NonNorm() = default;

    template <typename TCoeffs>
    BiQuadDF1NonNorm(TCoeffs&& coeffs, const T gain = 1.0)
        : m_coeffs(BiQuadCoeffsNorm<T>(coeffs) * gain)
        , m_a0(coeffs.a[0])
        , m_x(Eigen::Vector<T, 2>::Zero())
        , m_y(Eigen::Vector<T, 2>::Zero())
    ...
    T process_impl(T input)
    {
        ...
        const T out = calculate_biquad_df1_norm(m_coeffs, m_x, m_y, input);
...
private:
    BiQuadCoeffsNorm<T> m_coeffs;
    Eigen::Vector<T, 2> m_w;  // State variables w[n-1], w[n-2]

public:
    BiQuadDF2Norm() = default;
    ...
    T process_impl(T input)
    {
        ...
        const T feedback = precise_dot(m_coeffs.a, m_w);
        const T w        = input - feedback;

        // Calculate Output y[n]
        // Difference Equation: y[n] = b0*w[n] + b1*w[n-1] + b2*w[n-2]
        // Math: y[n] = (b0 * w[n]) + (b_tail . w_vector)

        const T feedforward_hist = precise_dot(m_coeffs.b.template tail<2>(), m_w);

        T out;
        if constexpr (std::is_floating_point_v<T>)
        {
            // High Precision: Combine the current w[n] term with the history sum
            out = std::fma(w, m_coeffs.b[0], feedforward_hist);
        }
        else
        {
            out = (w * m_coeffs.b[0]) + feedforward_hist;
        }

        // Shift State
        m_w[1] = m_w[0];  // w[n-2] = w[n-1]
        m_w[0] = w;       // w[n-1] = w[n]
...
public:
    ...
    T process_impl(T input)
    {
        ...
        const T feedback_sum = precise_dot(m_coeffs.a.template tail<2>(), m_w);
        const T w_acc        = input - feedback_sum;

        // Division by a0 happens here (Non-Normalizing behavior)
        const T w = w_acc / m_coeffs.a[0];
