...
#define DECLARE_DESCRIPTOR_PROPERTIES(TDescriptor)            \
    ...
    static constexpr auto kIsDiscrete = DescriptorT::kIsDiscrete;
...
struct SystemDescriptor
{
    ...
    static constexpr bool kIsReal     = tkIsReal;
    static constexpr bool kIsDiscrete = tkIsDiscrete;
    static constexpr char kDomainChar = kIsDiscrete ? 'z' : 's';
};
...
template <typename T>
concept CDiscreteSystemDescriptor = CSystemDescriptor<T> && (T::kIsDiscrete == true);

template <typename T>
concept CContinuousSystemDescriptor = CSystemDescriptor<T> && (T::kIsDiscrete == false);

template <typename T>
concept CRealSystemDescriptor = CSystemDescriptor<T> && (T::kIsReal == true);

template <typename T>
concept CComplexSystemDescriptor = CSystemDescriptor<T> && (T::kIsReal == false);
 ...
 */
template <typename T>
using DiscreteSystemDescriptorT = typename std::conditional<
    CDiscreteSystemDescriptor<T>,                           // Condition: Is T already discrete?
    T,                                                      // Type if true: Use T as is.
    SystemDescriptor<typename T::ValueT, T::kIsReal, true>  // Type if false: Convert to discrete.
    >::type;
 ...
 */
template <typename T>
using ContinuousSystemDescriptorT = typename std::conditional<
    CContinuousSystemDescriptor<T>,                          // Condition: Is T already continuous?
    T,                                                       // Type if true: Use T as is.
    SystemDescriptor<typename T::ValueT, T::kIsReal, false>  // Type if false: Convert to continuous.
    >::type;
