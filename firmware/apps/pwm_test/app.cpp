...
namespace valle::app
{
    std::atomic<uint32_t> g_rep_counter = 0;

    DriverBuilderReturnT install_drivers(DriverBuilderT&& builder)
    {
        return std::move(builder).template install<UARTLoggerT>().template install<HRTIMHalfBridgeDriverT>().yield();
    }
    ...
    static void init_drivers()
    {
        ...
        valle::expect(g_drivers.hb_driver.init(HRTIMHalfBridgeDriverConfig{
